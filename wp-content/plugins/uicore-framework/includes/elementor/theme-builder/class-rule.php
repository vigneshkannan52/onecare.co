		...
		if (is_array( $rules) && ! empty( $rules) ) {
			foreach ( $rules as $key => $rule ) {
				...
				switch ( $rule_case ) {
					...
					case 'special-portfolio':
						if ( \UiCore\Portfolio\Frontend::is_portfolio() && !is_singular() ) {
							$display = true;
						}
						break;
	...
	public function get_current_page_type() {
		if ( null === self::$current_page_type ) {
			...
			if ( is_404() ) {
				...
				} elseif ( \class_exists('\UiCore\Portfolio\Frontend') && \UiCore\Portfolio\Frontend::is_portfolio()) {
					$page_type = 'is_portfolios';
				}
			} elseif ( is_home() ) {
				$page_type = 'is_home';
	...
	public function get_posts_by_conditions() {
		...
		switch ( $current_page_type ) {
			...
			case 'is_home':
				...
				break;
			case 'is_portfolios':
				$meta_args .= " OR pm.meta_value LIKE '%\"special-portfolio\"%'";
				break;
			case 'is_front_page':
